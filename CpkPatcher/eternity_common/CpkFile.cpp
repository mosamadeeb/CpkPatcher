...
bool CpkFile::WriteItocEntry(uint32_t id, const CpkEntry &entry)
{
    ...
    if (!entry.GetExtractSize(&size))
        return false;

    entry.GetFileSize(&compressed_size);
    ...
    for (uint32_t i = 0; i < files_l; i++)
    {
        ...
        datal.SetWord("FileSize", (uint16_t)compressed_size, (unsigned int)i);
...
void CpkFile::UpdateToc()
{
    ...
    for (size_t i = 0; i < entries.size(); i++)
    {
        ...
        entry.GetExtractSize(&size);
        entry.GetFileSize(&compressed_size);

        toc.SetDword("FileSize", compressed_size, (unsigned int)i);
...
bool CpkFile::SaveCommon(Stream *stream)
{
    ...
    for (uint32_t i = 0; i < (uint32_t)entries.size(); i++)
    {
        ...
        if (!entry.GetFileSize(&size))
            return false;

        // Do not decompress when writing CPK
        if (!ExtractCommon(entry, stream, size, false))
            return false;
...
bool CpkFile::ExtractCommon(const CpkEntry &entry, Stream *stream, uint32_t size, bool decompress) const
{    
    if (entry.offset != (uint64_t)-1)
    {
        ...
        if (entry.compressed_size != 0 && decompress)
...
bool CpkFile::ExtractFile(uint32_t idx, const std::string &path, bool auto_path) const
{
    ...
    return ExtractCommon(entry, &output, size, true);
}
...
uint8_t *CpkFile::ExtractFile(uint32_t idx, uint64_t *psize) const
{
    ...
    if (!ExtractCommon(entry, &output, size, true))
    {
        delete[] buf;
        return nullptr;
    }    
...
bool CpkEntry::GetExtractSize(uint32_t *psize) const
{
...
bool CpkEntry::GetFileSize(uint32_t* psize) const
{
    if (offset != (uint64_t)-1 || buf)
    {
