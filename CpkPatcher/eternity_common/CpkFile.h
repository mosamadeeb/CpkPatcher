...
struct CpkEntry
{
    ...
    bool GetExtractSize(uint32_t *psize) const;
    bool GetFileSize(uint32_t* psize) const;
};
...
protected:
    ...
    bool ExtractCommon(const CpkEntry &entry, Stream *stream, uint32_t size, bool decompress) const;
...
public:
    ...
    virtual bool GetFileSize(uint32_t idx, uint64_t *psize) const
    {
        ...
        bool ret = entries[idx].GetExtractSize(&entry_size);
        if (ret)
            *psize = entry_size;
